...
### Standard Commands
- `npx claude-flow@alpha memory query <search>`: Search stored information
- `npx claude-flow@alpha memory stats`: Show memory usage statistics
...
### Batchtools Commands
- `npx claude-flow@alpha memory batch-store <entries-file>`: Store multiple entries in parallel
- `npx claude-flow@alpha memory parallel-query <queries-file>`: Execute multiple queries concurrently
- `npx claude-flow@alpha memory concurrent-export <namespaces>`: Export multiple namespaces simultaneously
- `npx claude-flow@alpha memory batch-cleanup <retention-config>`: Clean up multiple namespaces in parallel
...
npx claude-flow@alpha memory batch-store sparc-data.json --namespace sparc --parallel

# Concurrent query across multiple namespaces
npx claude-flow@alpha memory parallel-query "authentication design" --namespaces arch,impl,test

# Batch export with parallel compression
npx claude-flow@alpha memory concurrent-export project-backup --compress --parallel
```

### Performance Monitoring
```bash
# Monitor concurrent operations
npx claude-flow@alpha memory stats --concurrent --verbose

# Analyze batch operation performance
npx claude-flow@alpha memory performance-report --batchtools

# Check parallel indexing status
npx claude-flow@alpha memory index-status --parallel
```
...
npx claude-flow@alpha memory debug --concurrent

# Analyze batch operation performance
npx claude-flow@alpha memory analyze --batchtools --verbose

# Validate parallel index integrity
npx claude-flow@alpha memory index-validate --parallel --repair
```
